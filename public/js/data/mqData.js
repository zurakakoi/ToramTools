...
// Derived helper: split raw mqData into a flat list of quests with their
// chapter number, so the UI (and the MQ/Skip-MQ calculators) can group by
// "bab" (chapter) instead of by raw object key order.
export function buildQuestList() {
  const keys = Object.keys(mqData);
  const list = [];
  let chapter = 0;
  keys.forEach((key, index) => {
    ...
    list.push({
      ...
      chapter,
      name: key,
      xp: Number(mqData[key]),
    });
  });
  return list;
}

// Quests grouped per chapter (bab), each with total XP of that chapter.
export function buildChapters() {
  const quests = buildQuestList();
  const chapters = new Map();
  quests.forEach((q) => {
    if (!chapters.has(q.chapter)) {
      chapters.set(q.chapter, { chapter: q.chapter, quests: [], totalXp: 0 });
    }
    const c = chapters.get(q.chapter);
    c.quests.push(q);
    c.totalXp += q.xp;
  });
...
// Index of the special "Pre-Venena Metacoenubia Fight" quest (adds +12.5M xp
// if not skipped). Ported as-is from the original xp.js (index 85).
export const VENENA_QUEST_INDEX = 85;
export const VENENA_BONUS_XP = 12500000;
